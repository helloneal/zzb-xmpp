...
class XUser(cfgUser: String, setting: XUserSetting)(implicit bus: XmppEventBus) extends FSM[XPresence, XUserContext] {
    ...
    case Event(DoLogin, _) =>
      ...
      Future {
        ...
        case e: ConnectionException =>
          log.error(s"XUser <$cfgUser> xmpp connect to {} failed ", e.getFailedAddresses)
          self ! ConnectFailed
        case e: Throwable =>
          log.error(e, s"XUser <$cfgUser> connect failed")
          self ! ConnectFailed
      }
      goto(Connecting)
  ...
  whenUnhandled {
    case Event(value, stateData) ⇒
      log.warning(s"XUser <$cfgUser>  unhandled event " + value + " in state " + stateName)
      stay()
  }
